...
public class CartDao {
	...
	public List<CartBean> getAllCart(Integer userId) {
		return stmt.query(
				"select u.userId,p.productId,p.price,p.productName,p.quantity,c.cartId,c.deleted from cart c,product p,users u where c.productId = p.productId and c.deleted  = false",
	...
	public void addToCart(CartBean cart) {
		stmt.update("insert into cart (userId,productId,quantity) values (?,?,?) ", cart.getUserId(),
